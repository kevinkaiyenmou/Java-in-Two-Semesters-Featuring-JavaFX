 ...
 */
    public OptionalDouble withdrawMoney(String accountNumberIn, double amountIn) {
        BankAccount acc = getItem(accountNumberIn);
        if (acc != null) {
            if (acc.getBalance() >= amountIn) {
                acc.withdraw(amountIn);
                return OptionalDouble.of(acc.getBalance());
            } else {
                return OptionalDouble.of(acc.getBalance() - amountIn); // throw new insufficient funds exception (custom exception, will be
                           // caught/handled by the method caller)
            }
        } else {
            return /* there is no account! PANIC! */ OptionalDouble.empty(); // throw new no such account exception (custom exception, will
                                                         // be caught/handled by the method caller)
        }
    }
